...
import { priceFor, splitPayment, CURRENCY } from './prices.js';
...
export async function handleApplications(req, res) {
  if (req.method === 'OPTIONS') return corsPreflight(res);
  ...
  if (req.method === 'POST') {
    ...
    const amount = priceFor(formData.visaType);
    const { advance, balance } = splitPayment(amount);
...
export async function handleApplicationId(req, res) {
  if (req.method === 'OPTIONS') return corsPreflight(res);
